...
//Constructors
public Patient1(String id, String firstName, String lastName, String email, String gender, String bloodType, String mobileNumber,
...
//Other functions
public void Save_Patients(Vector<Patient1> save) {
	try {
		PrintWriter pout = new PrintWriter("Patients.txt");
...
public Vector<Patient1> Loader_Patients(){
	Vector<Patient1> temp=new Vector<Patient1>();
	try {
		Scanner pin = new Scanner(new FileReader("Patients.txt"));
		temp.add(new Patient1(id, firstName, lastName, email, gender, bloodType, mobileNumber, telephone, insuranceType, age, height, height));
		pin.close();
		return temp;
	}
...
public void Send_email(String ID) {
    ...
    try {
    	...
    	Scanner pat = new Scanner(new FileReader("Patients.txt"));
        ...
        while(pat.hasNext()) {
        	//Finding the patients first name and email
        	if (pat.next()==ID) {
        		pFname = pat.next();
        		pat.next();
        		recipient = pat.next();
